...
const skills = [
  ...
  { name: "Next.js", level: 75, icon: <SiNextdotjs size={32} className="text-gray-300" /> },
  { name: "Tailwind CSS", level: 85, icon: <SiTailwindcss size={32} className="text-teal-400" /> },
  { name: "Node.js", level: 70, icon: <FaNodeJs size={32} className="text-green-500" /> },
  { name: "Git", level: 75, icon: <FaGitAlt size={32} className="text-red-500" /> },
]
...
function Skills() {
  return (
    <section
      id="skills"
      className="scroll-mt-20 w-full bg-gray-900 text-white py-20 px-6 md:px-16"
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {skills.map((skill) => (
            <div
              key={skill.name}
              className="bg-white/10 backdrop-blur-md rounded-2xl p-6 flex flex-col items-center shadow-lg hover:scale-105 transition"
            >
              <div className="mb-4">{skill.icon}</div>
